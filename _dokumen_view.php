...
<link rel="stylesheet" href="resources/css/w3.css">
 <link rel="stylesheet" href="resources/pikaday/pikaday.css"> 
	
 <script src="resources/pikaday/moment.js"></script>
<script src="resources/pikaday/id.js"></script>
   
    <script src="resources/pikaday/pikaday.js"></script>
...
$sql_info="SELECT alur_perkara_id, jenis_perkara_id, jenis_perkara_nama FROM perkara WHERE perkara_id=".$perkara_id;$quer_info=mysql_query($sql_info); while($perkara_info=mysql_fetch_array($quer_info)) 
{
	...
	if($alur_perkara_id==15)
	{
	 ...
	 if($jenis_perkara_nama=="Cerai Talak" OR $jenis_perkara_nama=="Izin Poligami")
	 {
...
{ 
	...
	if($h_info["var_model"]<>"tanya_jawab")
	{
		...
		if($h_info["var_jenis"]=='textarea')		
		{
			...
			//$tinggi=6;
			?><textarea <?php echo @$fungsi?> style="width:750px;min-height:50px" rows="<?php echo $tinggi?>" id="<?php echo $h_info['var_nomor']?>" name="<?php echo $h_info['var_nomor']?>"><?php echo stripslashes(str_replace(";;",";",$isi))?></textarea>
			<?php 
		}else
	...
	}else
	{
		...
		?>
		<script>
			proses_tanya_jawab_ke_textarea(<?php echo $variabelnya; ?>);
		</script>
		<?php
	}		
}
...
<script type="text/javascript">  
  function myKeyPress(e,nomor,o,id, perkara_id, sidang_id, variabel){
	...
	if(keynum==114)
	{
		...
		var nomor_baru=nomor+1;
        Insert_Detail(o,nomor, perkara_id, sidang_id,  variabel); 
		return false;
	}
  }
</script>
...
function Insert_Detail(o, nomor, perkara_id, sidang_id, variabel)
{  
	var xhr = new XMLHttpRequest(); 
	var urutan_pertanyaan=nomor;
	xhr.open("POST","_dokumen_view_proses_tanya_jawab", true); 
	xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded"); 
	xhr.onreadystatechange = function() { 
		if(xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) { 
			...
			var a =insert_baris(o);
            a.innerHTML = '<td></td><td style="width:350px"><div id="q_tanya'+pesan+'" contenteditable="" class="tanya" onblur=Edit_Detail(this.innerHTML,"pertanyaan",'+pesan+','+variabel+')></div></td><td style="width:350px"><div id="q_jawab'+pesan+'" contenteditable="" class="jawab" onblur=Edit_Detail(this.innerHTML,"jawaban",'+pesan+','+variabel+') onkeydown="return myKeyPress(event,'+nomor+',this,'+pesan+','+perkara_id+','+sidang_id+','+variabel+')"></div></td>';	 
			
			document.getElementById('q_tanya'+pesan).focus();		
		}
	}
	xhr.send("insert_detail=ya&urutan_pertanyaan="+encodeURIComponent(urutan_pertanyaan)+"&perkara_id="+encodeURIComponent(perkara_id)+"&sidang_id="+encodeURIComponent(sidang_id)+"&saksi_id="+encodeURIComponent(variabel)); 	
}    
...
function copy_tanya_jawab(perkara_id, sidang_id, saksi_id, variabel_new)
{  
	var conf = confirm("Apakah anda yakin akan mengkopi tanya jawab ini ke tanya jawab saksi Lainnya?");
	if (conf == true) 
	{
		var xhr = new XMLHttpRequest(); 
		xhr.open("POST","_dokumen_view_proses_tanya_jawab", true); 
		...
		xhr.onreadystatechange = function() {//Call a function when the state changes.
			if(xhr.readyState == XMLHttpRequest.DONE && xhr.status == 200) {
				//document.getElementById("pesan_kirim").style="display.block" ;
				 ...
				 alert("Pastikan Anda Klik pada tabel tanya jawab, minimal 1 x");
                location.reload();
			}
		}
		xhr.send("copy_tanya_jawab=ya&saksi_id="+encodeURIComponent(saksi_id)+"&perkara_id="+encodeURIComponent(<?php echo $perkara_id?>)+"&sidang_id="+encodeURIComponent(<?php echo $id_sidang?>)+"&variabel_new="+encodeURIComponent(variabel_new)); 
	}	
}
